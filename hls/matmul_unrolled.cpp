...
#pragma HLS INTERFACE ap_ctrl_hs port=return
    ...
    // Fully partition into registers — needed for parallel access
    ...
    // Tiled multiplication with pipelining and unrolling
    TILE_ROW: for (int ii = 0; ii < N; ii += TILE) {
        TILE_COL: for (int jj = 0; jj < N; jj += TILE) {
            TILE_ACC: for (int kk = 0; kk < N; kk += TILE) {
                ...
                // Compute — pipelined + inner loop unrolled
                COMPUTE: for (int i = 0; i < TILE; i++) {
                    for (int j = 0; j < TILE; j++) {
                        #pragma HLS PIPELINE II=1
                        data_t sum = 0;
                        UNROLL_ACC: for (int k = 0; k < TILE; k++) {
                            #pragma HLS UNROLL
